(function() {
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    if (iterator === undefined) {
      ...
      _.each(array, function(item) {
        if (!myMap.has(iterator(item))) {
          myMap.set(iterator(item), item);
        }
      });
  ...
  _.every = function(collection, iterator) {
    ...
    if (iterator === undefined) {
      return _.reduce(testArr, function(acc, item) {
        return acc === false ? acc : Boolean(item);
      }, true);
    } else {
      return _.reduce(testArr, function(coll, item) {
        return Boolean(iterator(item)) && (coll === true);
      }, true);
    }
  };
  ...
  _.shuffle = function(array) {
    ...
    for (var i = 0; i < array.length; i++) {
      ...
      shuffled.push(copyArr[randomIndex]);
   ...
   //method2:
    ...
    var temp;
    _.each(newArr, function(item, i, newArr) {
